...
export function visibleSections(profile: Profile): readonly SectionDef[] {
  return sections.filter(({ id }) => {
    switch (id) {
      case 'news':
        return profile.news.length > 0;
      case 'publications':
        return profile.publications.length > 0;
      case 'research':
        return profile.research.length > 0;
      case 'skills':
        return profile.skills.length > 0;
      case 'projects':
        return profile.projects.length > 0;
      case 'internships':
        return profile.internships.length > 0;
      case 'education':
