...
| 方向 | 消息 | 说明 |
|---|---|---|
| 主 → iframe | `{t:'load', modelUrl, w, h}` | 加载 / 切换模型；`w/h` = 画布**后端缓冲**尺寸 |
| 主 → iframe | `{t:'pointer', x, y}` | **iframe 坐标内**的归一化位置，用于眼神跟随 |
| iframe → 主 | `{t:'ready'}` | 模型已就绪（父级可淡入） |
| iframe → 主 | `{t:'fail', reason}` | 加载失败，父级退回 SVG |

`load` 必须带上 `w/h`：**画布的后备缓冲尺寸决定取景** —— 运行时用
`setScreenRect(VIEW_LOGICAL_LEFT, VIEW_LOGICAL_RIGHT, -r, r)`（`r = canvasH / canvasW`）
定义可见范围，所以画布越扁、看到的纵向切片越窄。两个宿主必须完全同尺寸，
且尺寸**只有一份出处**（`pet.live2d.canvasW/canvasH`），渲染器刻意不留自己的默认值。
原委见 §8.1 的第 3 个 bug。
...
### 8.1 端到端验证结果（2026-09-12 完成；2026-09-14 复查并补 2 条断言 · 65 通过 / 0 失败 / 1 跳过）
...
| 2 | 两只都真渲染 | ✅ | 两只都进 `.pet--l2d`；主角画布 `id=pet-canvas-blanc`，同伴是 `IFRAME` 且内部 `#stage` 已就绪；**两只画布后端缓冲均为 384×576**（2026-09-14 补的断言，见下面第 3 个 bug） |
...
3. **同伴被放大 3 倍、头顶和小腿被切掉**（2026-09-14 用户拿截图报的）。
   这条是**存在性断言抓不到的**：当时「两只都进 `.pet--l2d`」「iframe 内 `#stage` 已就绪」
   全是绿的，但两只的取景不一致。

   **根因**：`render.html` 的 `#stage` 只写了 CSS `width/height: 100%`，
   **从未设这两个属性** —— 于是画布吃了 HTML 规范默认的 **300×150（2:1 横）**，
   而主页面画布是 **384×576（2:3 竖）**。

   **机制**（在 `live2d.js` 里查到的原文，不是推测）：

   ```js
   // 节选自运行时入口的 n(t)
   var i = C.width, e = C.height;   // C 就是 getElementById 拿到的那张画布
   var r = e / i;                   // r = 画布高 / 画布宽
   viewMatrix.setScreenRect(VIEW_LOGICAL_LEFT /* -1 */, VIEW_LOGICAL_RIGHT /* 1 */, -r, r);
   ```

   渲染链是 `identity → U(投影) → B(视图矩阵) → modelMatrix`（模型矩阵只按**模型自己的**
   画布尺寸归一化，与显示画布无关）。所以：**可见横向固定为 ±1，可见纵向 = ±(canvasH/canvasW)**。
   同伴那张 2:1 的画布可见纵向只剩 ±0.5 —— 恰好是模型高度的**中段三分之一**，
   表现就是放大 3 倍、头脚同时被切。

   **修法**：让尺寸只留一份出处。新增 `pet.live2d.canvasW / canvasH`（384×576），
   主角画布取它；`stage-iframe.ts` 在 `post()` 里随 `load` 一起把 `w/h` 发给渲染器；
   `render.html` 在 `loadlive2d` 之前设到画布上，**故意不保留自己的默认值** ——
   两边各写一份就迟早分叉，而这次的教训正是「默认值 300×150」悄悄生效。

   **回归断言**（已补进 e2e，修复前它读到 `主角 384×576 / 同伴 300×150`）：
   两只画布的后端缓冲尺寸必须完全一致，显示盒也必须一致。
